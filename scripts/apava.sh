...
# APAVA: fixed Medformer subject split, seed 42.
# Usage: CUDA_VISIBLE_DEVICES=0 bash scripts/apava.sh
python -u main.py \
  --datasets eeg \
  --dataset_names APAVA \
  ...
  --batch_size 8 \
  --random_seed 42 \
  --feature_cache_dir ./feature_cache/timemosaic_graph_seed42/apava \
  --result_dir ./results/timemosaic_graph_seed42/apava \
  "$@"
