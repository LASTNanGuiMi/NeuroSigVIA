...
RESULT_DIR="${RESULT_DIR:-results/eeg_medformer_adaptive_granularity_seed${SEED}/${DATASET_KEY}}"
FEATURE_CACHE_DIR="${FEATURE_CACHE_DIR:-feature_cache/eeg_medformer_adaptive_granularity_seed${SEED}/${DATASET_KEY}}"
...
echo "Dataset protocol: key=$DATASET_KEY dataset=$DATASET_NAME protocol=$PROTOCOL normalization=per_window_per_channel_standard_scaler_ddof0 metric_unit=window adaptive_granularity=1 bank=1-2-4,2-4-8,4-8-16"
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/neurosigvit_eeg_${DATASET_KEY}_mpl_gpu_${GPU}}"
export HF_HOME="${HF_HOME:-/tmp/neurosigvit_eeg_${DATASET_KEY}_hf}"
