...
# Shimmer10: HC versus MildPD/ModeratePD, fixed subject split.
# Usage: CUDA_VISIBLE_DEVICES=0 bash scripts/shimmer10.sh
python -u main.py \
  --datasets wearable \
  --dataset_names Shimmer_10_session10_AFC \
  --data_dir ./data/wearable \
  ...
  --batch_size 1 \
  --random_seed 42 \
  --feature_cache_dir ./feature_cache/timemosaic_graph_seed42/shimmer10 \
  --result_dir ./results/timemosaic_graph_seed42/shimmer10 \
  "$@"
