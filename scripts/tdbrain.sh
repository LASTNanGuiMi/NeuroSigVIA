...
# TDBRAIN: fixed Medformer legacy-50 subject split, seed 42.
# Usage: CUDA_VISIBLE_DEVICES=0 bash scripts/tdbrain.sh
python -u main.py \
  --datasets eeg \
  ...
  --batch_size 8 \
  --random_seed 42 \
  --feature_cache_dir ./feature_cache/timemosaic_graph_seed42/tdbrain \
  --result_dir ./results/timemosaic_graph_seed42/tdbrain \
  "$@"
